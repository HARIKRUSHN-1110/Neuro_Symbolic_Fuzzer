...
if __name__ == "__main__":
    ...
    # 1. Generate Scenario
    output_dir = os.path.abspath("resources")
    os.makedirs(output_dir, exist_ok=True)
    generated_files = s.generate(output_dir)
    xosc_file = generated_files[0][0]
    print(f"Scenario generated at: {xosc_file}")

    # 2. POINT TO YOUR ESMINI-DEMO FOLDER HERE
    # REPLACE THIS PATH with the actual path to your esmini.exe
    # Example: r"C:\Users\Dell\Downloads\esmini-demo\bin\esmini.exe"
    esmini_bin = r"C:\tools\esmini-demo\bin\esmini.exe" 

    # 3. Run
    if os.path.exists(esmini_bin):
        print(f"Found esmini at: {esmini_bin}")
        cmd = [
            esmini_bin,
        ...
        print(f"[ERROR] Still cannot find esmini at: {esmini_bin}")
        print("Please verify the path in the script matches your folder.")
